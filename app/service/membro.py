...
class MembroService:
    ...
    def generate_token(self, membro_id: str, celular: str):
        ...
        message = f"{EVOLUTION_API_UPDATE_MEMBER_MESSAGE}: {FRONTEND_URL}/update-member?token={token}"
    ...
    def new_member_generate_token(self, celular: str):
        ...
        message = f"{EVOLUTION_API_NEW_MEMBER_MESSAGE}: {FRONTEND_URL}/new-member?token={token}"
        self.evolution.send_message(celular, message)
        return token
